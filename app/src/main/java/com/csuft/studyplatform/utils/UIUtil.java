...
public final class UIUtil {
    ...
    public static void showToast(String desc){
        if (toast == null){
            toast = Toast.makeText(BaseApplication.getBaseApplication(),desc,Toast.LENGTH_SHORT);
        }else {
            toast.setText(desc);
        }
        toast.show();
    ...
    public static void showToast2(String desc){
        // 使用弱引用，防止内存泄漏
        if (toast == null){
            toast = Toast.makeText(BaseApplication.getBaseApplication(),desc,Toast.LENGTH_LONG);
        }else {
            toast.setText(desc);
        }
        toast.show();
